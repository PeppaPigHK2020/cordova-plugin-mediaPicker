...
var MediaPicker = {
    ...
    takePhoto: function (cameraOptions, success, error) {
        cameraOptions.destinationType = window.Camera.DestinationType.FILE_URI; //only support FILE_URI
    ...
    compressEvent: function (s, i) {
        cordova.fireDocumentEvent('MediaPicker.CompressVideoEvent', {
            'status': s,
            'index': i
        });
    },
    icloudDownloadEvent: function (p, i) {
        cordova.fireDocumentEvent('MediaPicker.icloudDownloadEvent', {
            'progress': p,
            'index': i
        });
    },
